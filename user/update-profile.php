...
if (isset($_POST)) {
	...
	if (isset($_FILES)) {
		...
		if (file_exists($_FILES['profile']['tmp_name'])) {
			...
			if ($resumeFileType == "jpg" || $resumeFileType == "png" || $resumeFileType == "jpeg"){

				if ($_FILES['profile']['size'] < 500000) { // File size is less than 5MB
	...
	$sql = "UPDATE users SET firstname='$firstname', lastname='$lastname', address='$address', city='$city', state='$state', contactno='$contactno', qualification='$qualification', stream='$stream', skills='$skills', aboutme='$aboutme', Hsc='$Hsc', Ssc='$Ssc', UG='$UG', PG='$PG',photo='$path'";
